        ...
        <div class="yz-panel">
          <div class="yz-panel-header">
            <div class="yz-panel-header-left">
              <span class="yz-panel-title">权限列表</span>
            </div>
          </div>
...
<script>
import permissionApi from '@/api/basic/permission'
...
export default {
  ...
  methods: {
    ...
    // 获取所有部门信息
    getPermissionAll() {
      return permissionApi.getPermissionAll().then(res => {
    ...
    deleteRow(index, row) {
      this.$confirm('你确定要删除该权限吗?', '提示', {
        ...
        permissionApi.deletePermission(row.id).then((res) => {
          if (res.status === 200) {
